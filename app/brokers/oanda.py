...
class MarketDataBuilder:
    ...
    def _calculate_usd_exposure(self, open_trades: list) -> str:
        ...
        if not open_trades:
            return "NONE"
        usd_pairs = [trade for trade in open_trades if "USD" in trade["instrument"]]
        if not usd_pairs:
            return "NONE"
        total_units = sum(abs(trade["units"]) for trade in usd_pairs)
...
def create_live_market_data_function():
    ...
    try:
        ...
        print(f"❌ OANDA connection failed: {exc}")
        return None
